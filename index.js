...
// fakedata upload
const fs = require('fs');
let fakeData = fs.readFileSync('./fakedata/details.json');
let datas = JSON.parse(fakeData);
// middleware
app.use(cors())
app.use(express.json());
//connection string
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@cluster0.nvfp0zy.mongodb.net/?retryWrites=true&w=majority`;
const client = new MongoClient(uri, { useNewUrlParser: true, useUnifiedTopology: true });
async function run() {
    try {
        await client.connect();
        const database = client.db('my-academy');
        const serviceCollection = database.collection('courses');
        ...
        // const result = await serviceCollection.insertMany(datas);
        // // res.json(result)
        // console.log(result);
        ...
        app.post('/courses', async (req, res) => {
            ...
            // const result = await serviceCollection.insertMany(fakedata.json);
            res.json(result)
            console.log(result);
        });
        //add user
...
app.get('/', (req, res) => {
    res.send('my-academy server is running');
})
